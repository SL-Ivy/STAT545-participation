...
| facetting             | no | (if yes, mutiple graphs will be produced)
...
```r
(mauna <- tsibble::as_tsibble(co2) %>%     # A tsibble is sorted by its key first and index.
   rename(month = index, conc = value)) # choose variables from a tbl to rename 
```
...
```r
ggplot(gapminder) +
  geom_point(aes(x = gdpPercap, y = lifeExp), alpha = 0.1) # x, y should be inclosed in ()
```
...
ggplot(mauna, aes(month(month), conc)) +
  geom_line(aes(group = year(month), colour = conc), alpha = 0.2) +  # change the colour by conc
  coord_polar() +
  scale_x_continuous(breaks = 1:12, 
                     labels = month.abb) +
  labs(x = "", 
       y = expression(paste(CO[2], " Concentration"))) +  # [] to change 2 as subscript
  theme_bw()
```
...
<img src="cm005-exercise_files/figure-html/unnamed-chunk-13-1.png" style="display: block; margin: auto;" />
